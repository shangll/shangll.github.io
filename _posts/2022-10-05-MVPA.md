...
至此，data预处理完成。我们可以正式开始***decoding***了。decoding最核心的原理就是把data拆成两个set，一个做traing，一个做testing。实际操作中，我们不可能只有两个data set，一般会用K-fold拆成几组——比如3组，然后用前两组test后一组，再用后两组test前一组，最后用两边test中间那组。每次test都会得到一个accuracy，得到3个ACC平均后就是decoding的正确率。这里其实可以用AUC（个人常用）。3组是Bae原文操作，我个人一般分10组，基本就很稳定了。
